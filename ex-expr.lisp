...
(defparameter grammar
  '(  
  (start --> stmt END temp      #'(lambda (stmts END temp) 
          (tac-to-rac (mk-code (var-get-code stmts))))
  )

  (temp --> entry END temp      #'(lambda (stmts) 
          (tac-to-rac (mk-code (var-get-code stmts))))) 
  (temp --> entry END       #'(lambda (stmts) 
          (tac-to-rac (mk-code (var-get-code stmts))))) 
  (temp -->        #'(lambda (temp) 
         )) 

  (stmts --> stmt END stmts             #'(lambda (stmts s) 
              (list (mk-place nil)
              (mk-code (append (var-get-code stmts)
                   (var-get-code s))))))
  (stmts --> stmt END             #'(lambda (stmts s) 
              (list (mk-place nil)
              (mk-code (append (var-get-code stmts)
                   (var-get-code s))))))  
  (entry --> stmt #'(lambda (stmts s) 
              (list (mk-place nil)
              (mk-code (append (var-get-code stmts)
                   (var-get-code s))))))
